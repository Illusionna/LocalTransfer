...
class BaseWSGIServer(HTTPServer):
    ...
    def log_startup(self) -> None:
        ...
        if self.address_family == af_unix:
            pass
            # messages.append(f"复制到浏览器 >>> {self.host} <<< 可省略 'http://'")
        else:
            ...
            if self.host in {"0.0.0.0", "::"}:
                ...
                # messages.append(f"复制到浏览器 >>> {scheme}://{localhost}:{self.port} <<< 可省略 'http://'")

            if ":" in display_hostname:
                display_hostname = f"[{display_hostname}]"

            # messages.append(f"复制到浏览器 >>> {scheme}://{display_hostname}:{self.port} <<< 可省略 'http://'")
...
def run_simple(
    ...
    if not is_running_from_reloader():
        srv.log_startup()
        _log("info", _ansi_style("按 CTRL+C 杀死进程以退出", "yellow"))
