...
test "ThreadPool: small fuzz" {
    ...
    var tp = try ThreadPool(testIncr).init(t.io, t.allocator, .{ .count = 3, .backlog = 3, .buffer_size = 512 });
...
test "ThreadPool: large fuzz" {
    testSum = 0; // global defined near the end of this file
    var tp = try ThreadPool(testIncr).init(t.io, t.allocator, .{ .count = 50, .backlog = 1000, .buffer_size = 512 });
...
fn testIncr(c: u64, buf: []u8) void {
    ...
    t.io.sleep(.fromMicroseconds(100), .awake) catch unreachable;
}
